...
class TestArtistList(TestCase):
    ...
    def test_list_types(self):
        # Test the artists list
        self.assertEqual(type(self.allartists.artists), type([]))
        self.assertEqual(type(self.allartists.artists[0]), type(()))
        self.assertEqual(type(self.allartists.artists[0][0]), int)
        self.assertEqual(type(self.allartists.artists[0][1]), str)

        # Test the artist_objects list
        self.assertEqual(type(self.allartists.artist_objects), type([]))
        # To test if the inside of the artist_objects list contains Artist instances, would need to
        # include the applayer.artist module in this file
        # self.assertEqual(type(self.allartists.artist_objects[0]), Artist)
